...
- **Thành viên 2**: Chu Đức Huy - MSSV: 1871020310

## Task division

- **Ma Huy Vũ phụ trách chính**: `secure_transfer_utils.py` (DES-CBC, RSA-OAEP, SHA-256, packet builder/parser), `keygen.py`, viết báo cáo và threat model.
- **Chu Đức Huy phụ trách chính**: `sender.py`, `receiver.py`, cấu hình socket TCP, viết test (`tests/`), thu thập log minh chứng.
- **Phần làm chung**: Thiết kế protocol packet, kiểm thử end-to-end, trả lời câu hỏi mở rộng.

## Demo roles

- **Demo Sender / mã hóa / log gửi**: Chu Đức Huy
- **Demo Receiver / giải mã / kiểm tra hash**: Ma Huy Vũ
- **Cả hai cùng trả lời câu hỏi mở rộng AES và chữ ký số**: Ma Huy Vũ Và Chu Đức Huy
...
```
[Máy Receiver - Vũ]                     [Máy Sender - Huy]
─────────────────────────────────────────────────────────────────
1. python keygen.py                         
...
2. python -m http.server 8080              
   (serve public key qua HTTP)             
                                            2. Tải public key:
                                               GET http://<IP vu>:8080/keys/receiver_public.pem
...
Script tự động:
- In ra IP máy Vũ để Huy biết
- Sinh RSA key pair nếu chưa có
- Mở HTTP server port 8080 để chia sẻ public key
- Bắt đầu lắng nghe socket port 6000

**Máy Huy (Sender) — chạy sau khi Receiver đã sẵn sàng:**

```powershell
.\run_sender.ps1 -ReceiverIP <IP_may_Vu>
```

Ví dụ nếu IP Vũ là `192.168.1.10`:
...
Script tự động:
- Tải `receiver_public.pem` từ máy vũ qua HTTP
- Dùng public key đó mã hóa DES key bằng RSA-OAEP
- Gửi packet qua TCP socket đến máy vũ

### Tìm IP máy Vũ (Receiver)
...
```powershell
# Tai public key tu may Vu
Invoke-WebRequest -Uri "http://<IP_Vu>:8080/keys/receiver_public.pem" -OutFile "keys/receiver_public.pem"

# Gui ban tin
$env:SERVER_IP="<IP_Vu>"; $env:DATA_PORT="6000"; $env:RECEIVER_PUBLIC_KEY="keys/receiver_public.pem"; $env:MESSAGE="Xin chao FIT4012! Lab 8 - Ma Huy Vu & Chu Duc Huy"; $env:SENDER_LOG_FILE="logs/sender_success.log"; python sender.py
```
